...
export function retrievePopularMovies(page) {
	console.log('tmdb----- url',Config.TMDB_URL);
	console.log(`${TMDB_URL}/movie/popular?api_key=${TMDB_API_KEY}&page=${page}`);
	return function (dispatch) {
...
export function retrieveNowPlayingMoviesSuccess(res) {
	console.log('retrieveNowPlayingMoviesSuccess')
	return {
...
export function retrieveNowPlayingMovies(page) {
	...
	console.log('heyyyyy',`${TMDB_URL}/movie/now_playing?api_key=${TMDB_API_KEY}&page=${page}`);
	return function (dispatch) {
